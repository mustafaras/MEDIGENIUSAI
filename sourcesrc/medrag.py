...
# Default OpenAI configuration - can be overridden by environment variables
openai.api_type = openai.api_type or os.getenv("OPENAI_API_TYPE") or "openai"
openai.api_version = openai.api_version or os.getenv("OPENAI_API_VERSION") or None
openai.api_key = openai.api_key or os.getenv('OPENAI_API_KEY') or ""

if openai.__version__.startswith("0"):
    openai.api_base = openai.api_base or os.getenv("OPENAI_API_BASE") or "https://api.openai.com"
    ...
    if openai.api_type == "azure":
        openai.azure_endpoint = openai.azure_endpoint or os.getenv("OPENAI_ENDPOINT") or ""
...
class MedRAG:
    ...
    def medrag_answer(self, question, options=None, k=32, rrf_k=100, save_dir = None, snippets=None, snippets_ids=None, **kwargs):
        ...
        if self.rag:
            ...
            contexts = ["Document [{}] with score {:.2f}".format(doc_id, score) for doc_id, score in retrieved_snippets]
            if len(contexts) == 0:
                contexts = [""]
            if "openai" in self.llm_name.lower():
